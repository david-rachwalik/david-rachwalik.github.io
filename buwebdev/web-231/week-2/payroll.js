...
(function () {
  ...
  const payRateRobinWilliams = 51.235;
  const hireDateRobinWilliams = `July 21, ${date.getFullYear()}`;

  const firstNameDaveGrohl = 'Dave';
  const lastNameDaveGrohl = 'Grohl';
  const addressDaveGrohl = '722 Seward Street';
  const payRateDaveGrohl = 32.657;
  const hireDateDaveGrohl = `January 14, ${date.getFullYear()}`;

  const firstNameKeanuReeves = 'Keanu';
  const lastNameKeanuReeves = 'Reeves';
  const addressKeanuReeves = '9601 Wilshire Blvd.';
  const payRateKeanuReeves = 47.624;
  const hireDateKeanuReeves = `September 2, ${date.getFullYear()}`;
