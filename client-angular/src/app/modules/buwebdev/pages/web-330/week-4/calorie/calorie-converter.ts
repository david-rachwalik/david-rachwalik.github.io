...
class CalorieConverter {
  ...
  static find(name: string): FoodModel[] {
    const results: FoodModel[] = [];
    const names = name.split(',');
    // for (const n of names) {
    //   const match = this.data.filter(
    //     (f) => String(f.name).toLowerCase() === String(n).trim().toLowerCase(),
    //   );
    ...
    Array.from(names).forEach((n) => {
      ...
      if (n && match.length > 0) results.push(match[0]);
    });
    return results;
  }
}
